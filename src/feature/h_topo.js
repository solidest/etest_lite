...
function _get_linking(doc, all_conns) {
    ...
    for(let l of raw) {
        ...
        linking.push({id: l.id, conns: new_conns});
    }
    return linking;
}
...
async function load(proj_id, topo_id) {
    ...
    for(let d of devs) {
        ...
        for(let c of d.conns) {
            conns.push({id: c.id, conn_obj: c, dev_obj: d});
        }
    }
    ...
    for(let d of mapping) {
        if(d.used === 'uut') {
            ...
            for(let c of dev.conns) {
                res.push({id: shortid.generate(), conns: [c.id]})
            }
        }
    }
    return res;
...
function get_conn_list(devs, mapping) {
    ...
    for(let map of mapping) {
        if(map.used!=='none') {
            ...
            for(let c of dev.conns) {
                res.push({text: `${dev.name}.${c.name} (${c.kind})`, value: c.id})
            }
        }
    }
    return res;
...
function get_draw_data(devs, mapping, linking, binding) {
    console.log(devs.length, mapping.length, linking.length, binding.length)
    return {
        // 点集
        nodes: [
          {
...
export default { load, get_dev_name, get_linking, get_conn_list, get_binding, get_draw_data }
