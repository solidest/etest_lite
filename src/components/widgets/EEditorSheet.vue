    ...
    <v-sheet tile class="mx-auto pa-2" :max-width="max_width?max_width:null">
        <v-row>
            ...
            <v-col v-for="(wed, idx) in widgets" :key="idx" :cols="wed.cols" class="pa-1">
                <v-text-field v-if="wed.type==='number'||wed.type==='text'" :label="hide_name?wed.label:`${wed.label}-${wed.name}`"
                    v-model="data[wed.name]" :disabled="wed.disabled && wed.disabled(wed.name, data)"
